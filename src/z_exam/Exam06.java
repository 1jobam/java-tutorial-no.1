...
//	(단, 모든 병사의 공격력과 방어력은 같아야 한다.)
	// weapon, armor, weaponUp, armorUp // 변하지 않아야 하는 값이므로
...
//			e. 생성자는 오버로딩 할 수 없다.
	//b = 객체를 초기화할 목적으로 사용된다. 객체를 생성하는 것은 new연산자이다., e = 생성자도 오버로딩이 가능하다.
...
//			d. 클래스 메서드 내에서는 사용할 수 없다.
	//b = 클래스 맴버(static이 붙은 변수나 메서드) 에는 사용할수 없다.
...
//			e.클래스변수보다 인스턴스변수가 먼저 초기화된다.
	//c = 초기화 블럭이 우선적으로 수행된다.,e = 클래스변수가 먼저 초기화된다.
...
//			e. 힙(heap)영역에 생성되며 가비지 컬렉터에 의해 소멸된다.
	//a = 지역변수는 자동초기화가 되지 않기때문에 적절한 값으로 초기화를 해야된다. ,
	//e = 힙 영역에는 인스턴스 가 생성되는 영역이며 지역변수는 콜스택에 생성된다.
...
//			f. 현재 실행중인 메서드는 println 뿐이다.
	//b = 나머지 메서드들은 종료된 것이 아니라 대기상태 이다.
...
//	}
//	라인 A, 라인 B, 라인 D
//	라인 A - static변수의 초기화에 인스턴스변수를 사용할 수 없다. 꼭 사용해야한다면, 객체를 생성해야한다.
